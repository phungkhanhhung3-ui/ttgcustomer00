...
import { Star, Package, Truck, ShieldCheck, Smile, BarChart3, MessageSquare, Search, Flame, AlertTriangle, QrCode, ClipboardList } from "lucide-react";
...
const initialReviews = [
  { orderCode:"TTG001245", customerName:"Nguyễn Minh Anh", status:"Đã giao", deliveryTime:5, staffAttitude:4, cargoSafety:5, overall:5, issueType:"Không có", note:"Giao đúng hẹn, nhân viên hỗ trợ tốt." },
  { orderCode:"TTG001246", customerName:"Công ty An Phát", status:"Đã giao", deliveryTime:4, staffAttitude:5, cargoSafety:4, overall:4, issueType:"Cập nhật trạng thái", note:"Hàng hóa an toàn, nên cập nhật trạng thái thường xuyên hơn." },
  { orderCode:"TTG001247", customerName:"Lê Hải Nam", status:"Đã giao", deliveryTime:3, staffAttitude:4, cargoSafety:4, overall:4, issueType:"Giao hàng chậm", note:"Nhìn chung ổn, nhưng giao hơi muộn." }
];

function Badge({ children, tone="red" }) { return <span className={`badge badge-${tone}`}>{children}</span>; }

function StarRating({ value, onChange }) {
  return <div className="stars">{[1,2,3,4,5].map(star =>
  ...
  )}<span className="subtle">{value}/5</span></div>;
}

function StatCard({ title, value, subtitle, Icon }) {
  return <div className="card stat-card">
    <div><div className="subtle">{title}</div><div className="stat-value">{value}</div><div className="subtle small">{subtitle}</div></div>
    <div className="stat-icon"><Icon size={22} /></div>
  </div>;
}

function FeatureItem({ Icon, children }) {
  return <div className="feature-item"><Icon size={16} /><span>{children}</span></div>;
}

function TTGLogo() {
  return <div className="ttg-logo">
    <div className="ttg-logo-icon"><Flame size={28} /></div>
    <div><div className="ttg-logo-text">TTG</div><div className="ttg-logo-sub">Logistics Service</div></div>
  </div>;
}
...
export default function App() {
  ...
  const [form, setForm] = useState({
    orderCode:"", customerName:"", status:"Đã giao", deliveryTime:5, staffAttitude:5, cargoSafety:5, overall:5, issueType:"Không có", note:""
  });
  const [lookupMessage, setLookupMessage] = useState("");
  const [search, setSearch] = useState("");

  useEffect(() => { window.localStorage.setItem("ttg-reviews-standalone", JSON.stringify(reviews)); }, [reviews]);
  ...
  const handleLookupOrder = () => {
    ...
    if (found) {
      setForm(prev => ({ ...prev, orderCode: found.orderCode, customerName: found.customerName, status: found.status }));
      setLookupMessage(`Đã tìm thấy đơn hàng ${found.orderCode} - ${found.customerName} (${found.status})`);
    } else setLookupMessage("Không tìm thấy mã đơn hàng trong danh sách demo.");
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.orderCode.trim() || !form.customerName.trim()) {
      setLookupMessage("Vui lòng nhập mã đơn hàng và tên khách hàng.");
      return;
    }
    setReviews(prev => [{ ...form }, ...prev]);
    setLookupMessage(Number(form.overall) <= 3 ? "Đánh giá đã được ghi nhận. Hệ thống đã đánh dấu phản hồi cần ưu tiên xử lý." : "Đánh giá đã được ghi nhận thành công.");
    setForm({ orderCode:"", customerName:"", status:"Đã giao", deliveryTime:5, staffAttitude:5, cargoSafety:5, overall:5, issueType:"Không có", note:"" });
  };

  return <div className="page"><div className="container">
    <div className="top-grid">
      <div>
        <div className="top-bar"><TTGLogo /><Badge>Giải pháp đề xuất cho TTG</Badge></div>
        <h1>Ứng dụng đánh giá chất lượng dịch vụ giao nhận khách hàng</h1>
        <p className="lead">Bản demo giúp khách hàng nhập mã đơn hàng, tra cứu trạng thái, chấm điểm dịch vụ, gửi góp ý và hỗ trợ doanh nghiệp theo dõi phản hồi, thống kê chất lượng cũng như cảnh báo các đánh giá thấp để xử lý kịp thời.</p>
      </div>
      <div className="card">
        ...
        <div className="feature-list">
          <FeatureItem Icon={Package}>Nhập và tra cứu mã đơn hàng, trạng thái đơn</FeatureItem>
          <FeatureItem Icon={Star}>Chấm điểm 4 tiêu chí dịch vụ theo thang 1–5</FeatureItem>
          <FeatureItem Icon={MessageSquare}>Gửi góp ý, đánh dấu loại vấn đề phát sinh</FeatureItem>
    ...
    <div className="main-grid">
      <div className="left-column">
        <div className="card">
          ...
          <div className="subtle small">Khách hàng hoàn thành sau khi đơn hàng được giao xong</div>
          <form className="form-grid" onSubmit={handleSubmit}>
            <div className="two-col">
              <div>
                <label>Mã đơn hàng</label>
                <div className="inline-inputs">
                  <input value={form.orderCode} onChange={(e) => setForm({ ...form, orderCode: e.target.value })} placeholder="Ví dụ: TTG001248" />
                  <button type="button" className="button soft" onClick={handleLookupOrder}>Tra cứu</button>
                </div>
                {lookupMessage ? <div className="subtle small mt8">{lookupMessage}</div> : null}
              </div>
              <div>
                <label>Tên khách hàng</label>
                <input value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} placeholder="Ví dụ: Công ty ABC" />
              </div>
            </div>
            ...
            {metrics.map(({ key, label, Icon }) => (
              <div key={key} className="metric-box">
                <div className="metric-head">
                  <div className="metric-left"><div className="metric-icon"><Icon size={18} /></div><div className="metric-label">{label}</div></div>
            ...
            <div>
              ...
              <select value={form.issueType} onChange={(e) => setForm({ ...form, issueType: e.target.value })}>
                <option>Không có</option><option>Giao hàng chậm</option><option>Hàng hóa hư hỏng</option><option>Thái độ phục vụ</option><option>Cập nhật trạng thái</option><option>Khác</option>
              </select>
            </div>

            <div>
              <label>Góp ý thêm</label>
              <textarea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="Khách hàng có thể ghi chú thêm về chất lượng dịch vụ..." />
            </div>

            <div className="two-col">
              <button type="submit" className="button primary">Gửi đánh giá</button>
              <button type="button" className="button secondary"><QrCode size={16} />QR đánh giá</button>
            </div>
          </form>
        </div>
      ...
      <div className="right-column">
        <div className="card">
          <div className="list-header">
            ...
            <div className="search-wrap">
              <Search size={16} />
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm theo mã đơn, tên khách hàng..." />
            </div>
          </div>
          ...
          <div className="review-list">
            ...
            {filteredReviews.length === 0 ? <div className="empty-box">Không có phản hồi nào khớp với từ khóa tìm kiếm.</div> : null}
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="urgent-list">
            {urgentReviews.length > 0 ? urgentReviews.map((review, index) => (
              <div key={`${review.orderCode}-urgent-${index}`} className="urgent-card">
                <div>
                  <div className="review-name">{review.customerName}</div>
                  <div className="subtle small">Mã đơn: {review.orderCode} • Vấn đề: {review.issueType}</div>
                </div>
